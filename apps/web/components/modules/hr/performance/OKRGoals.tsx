...
export function OKRGoals({ employeeId }: Props) {
  const { data: okrs, isLoading: _isLoading } = useGetPerformanceReviewsQuery({
    employeeId,
    type: "OKR",
  });
  ...
  const getHistoryData = (okr: any) => {
    ...
    return [
      ...
      {
        date: dayjs(okr.createdAt).add(1, "week").format("MMM DD"),
        progress: Math.round(okr.progress * 0.3),
      },
      {
        date: dayjs(okr.createdAt).add(2, "week").format("MMM DD"),
        progress: Math.round(okr.progress * 0.6),
      },
          ...
          extra={
            <Space size={16}>
              <Button
                ...
                onClick={() =>
                  setHistoryVisible(historyVisible === okr.id ? null : okr.id)
                }
              >
              ...
              <List.Item
                actions={[
                  <Button
                    ...
                    icon={<CheckOutlined />}
                    onClick={() =>
                      setCheckInModal({ open: true, keyResult: kr })
                    }
                  >
                    Check-in
                  </Button>,
      ...
      >
        ...
        >
          ...
          <Form.Item name="comment" label="Status Update / Comment">
            <Input.TextArea
              rows={3}
              placeholder="What has been achieved since last update?"
            />
          </Form.Item>
        </Form>
      </Modal>
