...
export function OnboardingPortal({ candidateId }: { candidateId: string }) {
  ...
  const currentStep =
    onboarding.status === "COMPLETED"
      ? 5
      : onboarding.tasks.filter((t: any) => t.isCompleted).length;
    ...
    <div style={{ maxWidth: 800, margin: "0 auto", padding: "40px 20px" }}>
      ...
      >
        <div style={{ textAlign: "center", marginBottom: 32 }}>
          ...
          <Paragraph>
            We are excited to have you on board. Please complete the following
            steps to finalize your onboarding.
          </Paragraph>
        </div>
          ...
          renderItem={(task: any) => (
            <List.Item
              extra={
                task.isCompleted ? (
                  <CheckCircleFilled
                    style={{ color: "#00b96b", fontSize: 24 }}
                  />
        ...
        {onboarding.status === "COMPLETED" && (
          ...
          >
            ...
            <Paragraph>
              Your documents have been verified. You will receive an email
