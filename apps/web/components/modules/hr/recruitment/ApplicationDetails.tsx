...
import { Card, Descriptions, Badge, Space, Button, List, Typography, Divider, Modal, Tag, Rate, Timeline, Row, Col, message } from "antd";
import { 
  CalendarOutlined, 
  FileTextOutlined, 
  CheckCircleOutlined, 
...
import { useGetApplicationQuery, useUpdateApplicationStatusMutation, useParseResumeMutation } from "@/store/api/recruitmentApi";
    ...
    <div style={{ padding: 20 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 24 }}>
        ...
        <Space>
          ...
          {(application.status === "INTERVIEW" || application.status === "SCREENED" || application.status === "TECHNICAL_TEST") && (
      ...
      <Card style={{ marginBottom: 24 }}>
        <Descriptions title="Candidate Information" column={3}>
          ...
          <Descriptions.Item label="Skills" span={2}>
            {application.candidate.skills?.map((s: string) => <Tag key={s} color="blue">{s}</Tag>) || "N/A"}
          </Descriptions.Item>
        </Descriptions>
      </Card>

      {application.notes && application.notes.includes("AI Score") && (
        <Card title="AI Insights" style={{ marginBottom: 24, borderColor: "#1677ff" }} bodyStyle={{ padding: "12px 24px" }}>
            <pre style={{ whiteSpace: "pre-wrap", margin: 0, fontFamily: "inherit", color: "#1677ff", fontWeight: 500 }}>{application.notes}</pre>
        </Card>
      )}
          ...
          <Card size="small">
            <Timeline
              ...
              items={(application.timeline || []).map((t: any) => ({
                ...
                color: t.status === "HIRED" ? "green" : t.status === "REJECTED" ? "red" : "blue",
              }))}
            />
          </Card>
        </Col>
