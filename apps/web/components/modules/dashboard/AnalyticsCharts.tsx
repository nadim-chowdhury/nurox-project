...
export function AnalyticsCharts({ dateRange }: Props) {
  ...
  const chartData = (data as any)?.data || data;
  const revenueGrowth = chartData?.revenueGrowth || [];
  const productivity = chartData?.productivity || [];
  const expenseBreakdown = chartData?.expenseBreakdown || [];
  const pipelineStats: any[] = chartData?.pipelineStats || [];
  const salesPerformance = chartData?.salesPerformance || [];
  const taskStats: any[] = chartData?.taskStats || [];
  ...
  const radarData = deptList.map((d: any) => ({
    subject: d.name,
    A: d.tasks,
    B: d.employees * 10,
    fullMark: 150,
            ...
            <div style={{ width: "100%", height: 250 }}>
              <ResponsiveContainer>
                <PieChart>
                  ...
                  >
                    {pipelineStats.map((entry: any, index: number) => (
                      <Cell
                        key={`cell-${index}`}
            ...
            <div style={{ width: "100%", height: 300 }}>
              <ResponsiveContainer>
                <BarChart
                  data={taskStats}
                  onClick={(e: any) =>
                    e && handleBarClick(e.activePayload?.[0]?.payload)
                  }
                  style={{ cursor: "pointer" }}
                >
                  <CartesianGrid
