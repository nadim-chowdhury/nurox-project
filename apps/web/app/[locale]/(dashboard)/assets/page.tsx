...
export default function AssetsPage() {
  ...
  const netBookValue = assets.reduce(
    (sum: number, a: any) =>
      sum + Number(a.netBookValue || a.purchaseCost || 0),
    0,
  );
  ...
  const columns = [
      ...
      render: (text: string) => (
        <Tag
          ...
          style={{
            ...
            color: "var(--color-primary)",
            borderColor: "rgba(195, 245, 255, 0.3)",
          }}
        >
          {text || "Uncategorized"}
        </Tag>
    ...
    },
    {
      ...
      render: (text: string, record: any) =>
        record.assignedEmployee ? (
          ...
          >{`${record.assignedEmployee.firstName} ${record.assignedEmployee.lastName}`}</Text>
        ) : (
    ...
    },
    {
      ...
      key: "netBookValue",
      render: (val: number | string, record: any) => (
        <Text
          className="font-display"
          style={{ color: "var(--color-success)", fontWeight: 600 }}
        >
          ${Number(val || record.purchaseCost || 0).toFixed(2)}
        </Text>
      ),
    },
    {
      ...
      <Row gutter={[16, 16]} style={{ marginBottom: 24 }}>
        ...
        <Col xs={12} sm={6}>
          ...
          >
            <KpiCard
              title="Net Book Value"
              value={`$${netBookValue.toLocaleString(undefined, { minimumFractionDigits: 2 })}`}
            />
          </Skeleton>
        </Col>
        <Col xs={12} sm={6}>
