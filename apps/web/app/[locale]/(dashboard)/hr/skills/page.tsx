...
export default function SkillMatrixPage() {
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (_: any, record: any) => {
        const lowProficiency = record.employees.filter(
          (e: any) => e.proficiency < 4,
        ).length;
        ...
        return (
          <Space direction="vertical" size={0} style={{ width: "100%" }}>
            <Text
              type={gapPercent > 50 ? "danger" : "secondary"}
              style={{ fontSize: 12 }}
            >
            ...
            <Progress
              ...
              showInfo={false}
              strokeColor={
                gapPercent > 50 ? "var(--color-error)" : "var(--color-primary)"
              }
            />
          </Space>
        );
  ...
  const dataSource = matrix
    ? Object.keys(matrix).map((skillName) => ({
        key: skillName,
        skillName,
