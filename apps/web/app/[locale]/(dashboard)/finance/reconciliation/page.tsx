...
export default function BankReconciliation() {
  ...
  const { data: journalLines, isLoading: journalsLoading } =
    useGetUnreconciledJournalsQuery(selectedBankAccountId!, {
      skip: !selectedBankAccountId,
    });

  const [reconcile] = useReconcileTransactionMutation();

  const [selectedBankTrxId, setSelectedBankTrxId] = useState<string | null>(
    null,
  );
  const [selectedJournalLineId, setSelectedJournalLineId] = useState<
    string | null
  >(null);
  ...
  const unreconciledBankTrx =
    bankTrx?.filter((t) => t.status === "UNRECONCILED") || [];
    ...
    <div className="animate-fade-in-up">
        ...
        <Col span={12}>
          ...
          >
            <Table
              ...
              rowSelection={{
                type: "radio",
                selectedRowKeys: selectedJournalLineId
              ...
              columns={[
                ...
                { title: "Ref", dataIndex: ["journalEntry", "entryNumber"] },
                {
                  ...
                  render: (_, record) =>
                    formatCurrency(
                      Number(record.debit) - Number(record.credit),
                    ),
                },
              ]}
            />
      ...
      {!selectedBankTrxId && !selectedJournalLineId && (
        <div className="mt-4 bg-blue-50 p-4 rounded text-blue-700 text-center">
          <p>
            Select a bank transaction and a corresponding journal entry to
            reconcile them.
          </p>
        </div>
      )}
